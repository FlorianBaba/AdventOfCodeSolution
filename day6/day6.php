...
function getAnswer($string, $part = 1)
{
    // init the grid
    $grid = array();
    actionLight($grid, 0, 0, 999, 999, 'off', 1);

    echo 'Search for part '.$part.' in progress ...'.PHP_EOL;

    foreach (explode(PHP_EOL, $string) as $lign => $instruction) {
        ...
        $xStart = getXFromCoord($coords[0]);
        $yStart = getYFromCoord($coords[0]);
        $xEnd = getXFromCoord($coords[1]);
        $yEnd = getYFromCoord($coords[1]);
        ...
        // do the action !
        actionLight($grid, $xStart, $yStart, $xEnd, $yEnd, $action, $part);
    }

    return 'The number of lit lights is '.array_sum($grid);
}
 ...
 * @throws ErrorException
 */
function actionLight(array &$grid, $xStart, $yStart, $xEnd, $yEnd, $action, $part)
{
    ...
    for ($x = $xStart; $x <= $xEnd; $x++) {
        for ($y = $yStart; $y <= $yEnd; $y++) {
            if ($part === 1) {
                actionLightForPart1($grid, $x, $y, $action);
            } else {
                actionLightForPart2($grid, $x, $y, $action);
            }
        }
    }
}
 ...
 * @param $action
 */
function actionLightForPart1(array &$grid, $x, $y, $action)
{
    switch ($action) {
        case 'on' :
            $grid[$x.','.$y] = 1;
 ...
 * @param $action
 */
function actionLightForPart2(array &$grid, $x, $y, $action)
{
    switch ($action) {
        case 'on' :
                $grid[$x.','.$y]++;
 ...
 * @return int
 */
function getYFromCoord($coord)
{
    return array_pop(explode(',', $coord));
}

echo '***** Solution - Day 6 *****'.PHP_EOL.PHP_EOL;
